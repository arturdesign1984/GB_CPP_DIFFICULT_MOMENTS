...
TEST_F(PhoneBookTest, GetPhoneNumber)
{
	ASSERT_EQ(std::get<1>(book->GetPhoneNumber("Ильин")), PhoneNumber(7, 17, "455767", 57));
	ASSERT_EQ(std::get<0>(book->GetPhoneNumber("Абракадабра")), "not found");
}
TEST_F(PhoneBookTest, ChangePhoneNumber)
{
	book->ChangePhoneNumber(Person("Ильин Петр Артемович"),PhoneNumber(1, 23, "12345", 67));
	ASSERT_EQ(std::get<1>(book->GetPhoneNumber("Ильин")), PhoneNumber(1, 23, "12345", 67));
	book->ChangePhoneNumber(Person("Ильин Петр Артемович"),PhoneNumber(8, 76, "54321", std::nullopt));
	ASSERT_EQ(std::get<1>(book->GetPhoneNumber("Ильин")), PhoneNumber(8, 76, "54321", std::nullopt));
}
TEST_F(PhoneBookTest, SortByName)
{
	...
	str = str.substr(count, (sizeof("Александров") / sizeof(char)) - 1);
	ASSERT_TRUE(str == "Александров");
}
TEST_F(PhoneBookTest, SortByPhone)
{
	...
	str = str.substr(count, (sizeof("Соловьев") / sizeof(char)) - 1);
	ASSERT_TRUE(str == "Соловьев");
}
